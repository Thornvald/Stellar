...
const WINDOWS_MANIFEST_DIR = path.join(process.env.ProgramData ?? 'C:\\ProgramData', 'Epic', 'EpicGamesLauncher', 'Data', 'Manifests');
        ...
        for (const launcherPath of WINDOWS_LAUNCHER_PATHS) {
            try {
                ...
                return data.InstallationList
                    ...
                    .map((entry) => {
                    const name = entry.DisplayName || entry.AppName || path.basename(entry.InstallLocation ?? 'Unreal Engine');
                    return {
                        name,
...
export async function detectUnrealEngineInstalls() {
