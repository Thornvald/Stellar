                    ...
                    if let Some(list) = data.get("InstallationList").and_then(|v| v.as_array()) {
                        for item in list {
                            if let Some(location) = item.get("InstallLocation").and_then(|v| v.as_str()) {
                                ...
                                let version = item.get("AppVersion")
                                    .and_then(|v| v.as_str())
                                    .map(|s| s.to_string())
...
fn start_build(
    ...
    // Find UAT script
    #[cfg(windows)]
    let uat_script = PathBuf::from(&unreal_engine_path)
    ...
    if !uat_script.exists() {
        return Err(format!("UAT script not found at {:?}", uat_script));
    }

    let mut cmd = Command::new(&uat_script);
    ...
    state.builds.lock().unwrap().insert(build_id.clone(), build_process);
...
fn get_build_status(state: State<AppState>, build_id: String) -> Result<BuildStatus, String> {
    ...
    let build = builds.get_mut(&build_id)
            ...
            match child.try_wait() {
                Ok(Some(exit_status)) => {
                    ...
                    if exit_status.success() {
                        ...
                        build.status.error = Some(format!("Process exited with code {:?}", exit_status.code()));
                    }
                }
                Ok(None) => {
                    // Still running
...
#[tauri::command]
fn get_build_logs(state: State<AppState>, build_id: String, from: usize) -> Result<BuildLogsResponse, String> {
    let mut builds = state.builds.lock().unwrap();
    ...
    let lines: Vec<String> = build.logs.iter().skip(from).cloned().collect();
    let next_index = build.logs.len();
