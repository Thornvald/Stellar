...
var ConfigSchema = external_exports.object({
  ...
  unrealEnginePath: external_exports.string().min(1).nullable()
});
var BuildStartSchema = external_exports.object({
  projectPath: external_exports.string().min(1),
  unrealEnginePath: external_exports.string().min(1)
});
var BuildIdParamSchema = external_exports.object({
  id: external_exports.string().min(1)
});
var BuildLogsQuerySchema = external_exports.object({
...
// src/services/engineDetector.js
var import_node_fs = require("node:fs");
var import_node_path = __toESM(require("node:path"), 1);
var import_node_os = __toESM(require("node:os"), 1);
var WINDOWS_BASE_DIRS = [
  process.env.PROGRAMFILES,
  process.env["PROGRAMFILES(X86)"]
].filter((value) => Boolean(value)).map((base) => import_node_path.default.join(base, "Epic Games"));
var WINDOWS_MANIFEST_DIR = import_node_path.default.join(process.env.ProgramData ?? "C:\\ProgramData", "Epic", "EpicGamesLauncher", "Data", "Manifests");
var WINDOWS_LAUNCHER_PATHS = [
  import_node_path.default.join(process.env.ProgramData ?? "C:\\ProgramData", "Epic", "UnrealEngineLauncher", "LauncherInstalled.dat"),
  import_node_path.default.join(process.env.ProgramData ?? "C:\\ProgramData", "Epic", "EpicGamesLauncher", "LauncherInstalled.dat")
];
var DARWIN_BASE_DIRS = [
  import_node_path.default.join("/Users", "Shared", "Epic Games"),
  import_node_path.default.join(import_node_os.default.homedir(), "Epic Games")
];
var LINUX_BASE_DIRS = [
  import_node_path.default.join(import_node_os.default.homedir(), "Epic Games"),
  import_node_path.default.join(import_node_os.default.homedir(), ".local", "share", "Epic Games"),
  import_node_path.default.join("/opt", "Epic Games")
];
var SKIP_DIRECTORY_NAMES = /* @__PURE__ */ new Set([
  "launcher",
  "epicgameslauncher",
  "epic games launcher",
...
async function dirExists(dirPath) {
  try {
    const stat = await import_node_fs.promises.stat(dirPath);
...
async function listCandidates(baseDir) {
  try {
    if (!await dirExists(baseDir)) {
      return [];
    }
    const entries = await import_node_fs.promises.readdir(baseDir, { withFileTypes: true });
    return entries.filter((entry) => entry.isDirectory() && !shouldSkipDirectory(entry.name)).map((entry) => ({
      name: entry.name,
      path: import_node_path.default.join(baseDir, entry.name),
...
function parseVersionFromName(name) {
  const match2 = name.match(/UE[_-]([0-9]+(?:\.[0-9]+)*)/i);
  if (match2 && match2[1]) {
    return match2[1];
  }
...
async function isEngineRoot(candidatePath) {
  const engineDir = import_node_path.default.join(candidatePath, "Engine");
  if (!await dirExists(engineDir)) {
    return false;
  }
  const binariesDir = import_node_path.default.join(engineDir, "Binaries");
  const buildDir = import_node_path.default.join(engineDir, "Build");
...
async function readJsonFile(filePath) {
  try {
    const raw2 = await import_node_fs.promises.readFile(filePath, "utf8");
    return JSON.parse(raw2);
  } catch (err) {
    return null;
  }
}
function formatLabel(name, version) {
  if (version) {
...
async function listManifestCandidates() {
  if (process.platform !== "win32") {
    return [];
  }
  try {
    ...
    const entries = await import_node_fs.promises.readdir(WINDOWS_MANIFEST_DIR, { withFileTypes: true });
    const manifests = entries.filter((entry) => entry.isFile() && entry.name.endsWith(".item"));
    const candidates = [];
    for (const entry of manifests) {
      try {
        const filePath = import_node_path.default.join(WINDOWS_MANIFEST_DIR, entry.name);
        const data = await readJsonFile(filePath);
        if (!data || !data.InstallLocation) {
          continue;
        }
        const name = data.DisplayName || data.AppName || import_node_path.default.basename(data.InstallLocation);
        candidates.push({
          name,
...
async function listLauncherCandidates() {
  if (process.platform !== "win32") {
    return [];
  }
  try {
    for (const launcherPath of WINDOWS_LAUNCHER_PATHS) {
      try {
        const data = await readJsonFile(launcherPath);
        if (!data || !Array.isArray(data.InstallationList)) {
          continue;
        }
        return data.InstallationList.filter((entry) => Boolean(entry.InstallLocation)).map((entry) => {
          const name = entry.DisplayName || entry.AppName || import_node_path.default.basename(entry.InstallLocation ?? "Unreal Engine");
          return {
            name,
...
async function detectUnrealEngineInstalls() {
  ...
  for (const candidate of candidates) {
    try {
      const dirName = import_node_path.default.basename(candidate.path);
      if (shouldSkipDirectory(dirName)) {
        continue;
      }
      const normalizedPath = import_node_path.default.normalize(candidate.path);
...
// src/app.js
function registerRoutes(app3) {
  return app3.use("/api/*", cors({
    origin: "*",
    allowHeaders: ["Content-Type"],
    allowMethods: ["GET", "POST", "OPTIONS"]
  })).get("/api/health", (c) => c.json({ ok: true })).get("/api/config", async (c) => {
  ...
  }).get("/api/build/:id/status", zValidator("param", BuildIdParamSchema), async (c) => {
    const services = c.get("services");
  ...
  }).get("/api/build/:id/logs", zValidator("param", BuildIdParamSchema), zValidator("query", BuildLogsQuerySchema), async (c) => {
    const services = c.get("services");
    const { id } = c.req.valid("param");
    ...
    return c.json(logs);
  }).post("/api/build/:id/cancel", zValidator("param", BuildIdParamSchema), async (c) => {
    const services = c.get("services");
    const { id } = c.req.valid("param");
...
var app = registerRoutes(new Hono2());
function createApp(services) {
  let instance = new Hono2();
...
async function dirExists2(dirPath) {
  try {
    const stat = await import_node_fs2.promises.stat(dirPath);
    return stat.isDirectory();
  } catch {
    return false;
  }
}
async function findEditorTarget(sourceDir) {
  const entries = await import_node_fs2.promises.readdir(sourceDir, { withFileTypes: true });
...
async function resolveTargetName(projectPath) {
  const projectDir = import_node_path2.default.dirname(projectPath);
  const sourceDir = import_node_path2.default.join(projectDir, "Source");
  ...
  const projectName = import_node_path2.default.basename(projectPath, import_node_path2.default.extname(projectPath));
  return `${projectName}Editor`;
}
function buildUbtPath(unrealEnginePath) {
...
function finalizeJob(job, status, code, error) {
  if (job.finishedAt) {
    return;
  }
  job.status = status;
...
function createBuildManager() {
  ...
  async function startBuild(payload) {
    ...
    job.process = child;
    const stdoutReader = import_node_readline.default.createInterface({ input: child.stdout });
    stdoutReader.on("line", (line) => appendLog(job, line));
    const stderrReader = import_node_readline.default.createInterface({ input: child.stderr });
...
function getConfigDir() {
  if (process.platform === "win32") {
    return process.env.APPDATA ?? import_node_path3.default.join(import_node_os2.default.homedir(), "AppData", "Roaming");
  }
  if (process.platform === "darwin") {
    return import_node_path3.default.join(import_node_os2.default.homedir(), "Library", "Application Support");
  }
  return process.env.XDG_CONFIG_HOME ?? import_node_path3.default.join(import_node_os2.default.homedir(), ".config");
}
function getConfigPath() {
  return import_node_path3.default.join(getConfigDir(), APP_FOLDER, CONFIG_FILE);
}
function normalizeConfig(value) {
  ...
  const raw2 = value;
  const projects = Array.isArray(raw2.projects) ? raw2.projects : [];
  ...
  const unrealEnginePath = typeof raw2.unrealEnginePath === "string" && raw2.unrealEnginePath.trim() ? raw2.unrealEnginePath.trim() : null;
  return {
...
async function ensureDirExists(filePath) {
  ...
  await import_node_fs3.promises.mkdir(dir, { recursive: true });
}
function createConfigStore() {
  ...
  return {
    ...
    async load() {
      try {
        const contents = await import_node_fs3.promises.readFile(configPath, "utf8");
    ...
    async save(config) {
      ...
      await import_node_fs3.promises.writeFile(configPath, payload, "utf8");
    }
  };
}
...
var buildManager = createBuildManager();
var app2 = createApp({ configStore, buildManager });
var port = Number.parseInt(process.env.STELLAR_RPC_PORT ?? String(RPC_PORT), 10) || RPC_PORT;
var hostname = process.env.STELLAR_RPC_HOST ?? RPC_HOST;
async function isPortAvailable(host, portNum) {
  return new Promise((resolve) => {
    const tester = (0, import_node_net.createServer)().once("error", () => resolve(false)).once("listening", () => {
      tester.close(() => resolve(true));
    }).listen(portNum, host);
  });
}
async function waitForPort(host, portNum, maxAttempts = 10, delayMs = 500) {
